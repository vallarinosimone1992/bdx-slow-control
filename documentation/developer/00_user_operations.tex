...
The main startup command checks whether \pv{BDX:ENV:TEMP:T00:VALUE} is reachable. Start the remote service when required:
...
The command connects through SSH, runs the appropriate systemd action, and waits for the readiness PV. If it fails, inspect the remote service:
...
\begin{enumerate}
  \item validates the main-host address and local installation;
  ...
  \item waits for the IOC and a representative PV;
...
Startup does not enable PSU outputs, start or stop the chiller, or write setpoints. A normal launch leaves the \textbf{BDX Main IOC} and \textbf{BDX Archiver and Phoebus} terminals open for inspection.
...
Normal shutdown is graceful. Use \code{--force} only when a recognized process does not terminate normally.
...
\addlinespace
Live values but no history & Confirm Archiver registration, connection state, retrieval availability, selected time range, and synchronized clocks. \\
\addlinespace
