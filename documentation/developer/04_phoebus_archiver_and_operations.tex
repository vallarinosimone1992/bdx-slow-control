...
\begin{enumerate}
  \item update the appropriate repository PV list;
  \item run offline tests and PV-list validation;
  \item deploy or register the list through the controlled helper;
  \item confirm \code{connectionState=true} for a representative PV;
  \item verify live and historical retrieval in Phoebus;
  \item document that samples exist only from the registration time onward.
\end{enumerate}

Keep host clocks synchronized. Data Browser retrieval is timestamp-based, and clock skew can make recent samples appear absent.

\section{Operator command layer}

The installed Ubuntu commands provide a constrained operator workflow:

\begin{itemize}
  \item \command{bdx_slow_control_start};
  \item \command{bdx_slow_control_kill_ioc};
  \item \command{bdx_slow_control_kill_archiver};
...
Shutdown commands target only recognized BDX slow-control processes. They use graceful termination by default and reserve forced killing for an explicit \code{--force}. Software process shutdown must remain separate from hardware state changes.
