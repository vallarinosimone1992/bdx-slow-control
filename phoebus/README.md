...
Default launch behavior is live-only:

```bash
BDX_ARCHIVER_ENABLED=false ./scripts/launch_phoebus.sh overview
```

Enable the EPICS Archiver Appliance by providing the retrieval endpoint:
...
To generate `.plt` files that explicitly contain the archive source:
