...
BDX_PHOEBUS_UPDATE_THROTTLE_MS="${BDX_PHOEBUS_UPDATE_THROTTLE_MS:-1000}"
if [[ ! "$BDX_PHOEBUS_UPDATE_THROTTLE_MS" =~ ^[0-9]+$ ]] || \
   (( BDX_PHOEBUS_UPDATE_THROTTLE_MS < 1000 )); then
    echo "BDX_PHOEBUS_UPDATE_THROTTLE_MS must be an integer at least 1000 ms." >&2
    exit 2
fi
export BDX_TREND_RANGE="${BDX_TREND_RANGE:-10 minutes}"
export BDX_TREND_SCAN_PERIOD="${BDX_TREND_SCAN_PERIOD:-1.0}"
BDX_TREND_RING_SIZE="${BDX_TREND_RING_SIZE:-5000}"
BDX_ARCHIVER_ENABLED="${BDX_ARCHIVER_ENABLED:-true}"
BDX_ARCHIVER_URL="${BDX_ARCHIVER_URL:-http://127.0.0.1:17668/retrieval}"
